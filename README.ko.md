...
> **한 줄 요약:** Claude Code나 Codex에 주제를 던지면, venue 영향력(IF) 기반으로 랭킹된
> 중복제거·출처추적 단일 논문 코퍼스를 PDF까지 디스크에 담아 돌려준다.
...
- 🏆 **Venue 영향력(IF) 우선** — 6번째 랭킹 레이어가 각 논문을 발표 venue의 IF로 채점한다: BK21 목록의 **CS/AI 우수 학회 188개**(인정 IF 1~4; AAAI·CVPR·NeurIPS·ICML·ACL …, +ICLR·COLM)와 **OpenAlex 실시간 저널 IF**(JIF 등가 지표; TPAMI·JMLR·Nature MI …). `--goal impact`로 IF 높은 venue의 논문을 상위로 올린다.
- 🧵 **서브에이전트 팬아웃** — 소스 버킷당 검색 에이전트 1개를 병렬 실행 → 넓은 커버리지를 직렬 대기 없이.
- 🧹 **출처를 동반한 중복제거** — DOI → arXiv-id → 정규화 제목으로 병합하고, 각 논문이 *어떤* DB에서 나왔는지 기록.
- 📊 **6층 랭킹** — 출처 교차검증·인용 영향력·최신성·접근성·주제 관련성·venue 영향력으로 채점 → SEO가 아니라 실질로 상위 결정.
- 📄 **원본 PDF** — 상위 K편을 오픈액세스 경로로 자동 확보, 받은 것·페이월 폴백 필요한 것을 manifest로 기록.
- 🧭 **AI 인식 라우팅** — `ai-research` 프로필이 ML/CS에 맞는 DB(arXiv·Semantic Scholar·DBLP·OpenAlex·GitHub)를 선택하고, 물리·생명과학·암호·경제·수학도 그대로 커버.
...
# 완료된 실행 디렉토리를 self-contained HTML 리포트로 렌더링
python3 ~/.claude/skills/scholar-megasearch/scripts/render_artifact.py \
  ./literature_search/<주제>_<날짜>

# HTML 리포트를 http://127.0.0.1:8765/ 에서 미리보기
python3 ~/.claude/skills/scholar-megasearch/scripts/render_artifact.py \
  ./literature_search/<주제>_<날짜> --serve --open
...
기본 병합 랭킹은 **6층** 가중 점수다: provenance, impact, recency, access/completeness,
topic relevance, **venue impact(IF)**. 목표 프로필(`survey`, `systematic`, `newest`,
`seminal`, `implementation`, `pdf-corpus`, **`impact`**)이 각 레이어 가중치를 조절하되
신호는 분리한다. 이전 `sources_count, citations, year` 정렬을 재현하려면
`--ranking classic`을 쓴다.

### Venue impact (IF) 우선 — AI 연구자용

발표 venue의 영향력(IF)을 6번째 랭킹 레이어로 반영해 **IF 높은 곳의 논문을 상위로** 올린다.
두 개의 IF 표를 쓴다:

- **BK21 학회표** (`references/venue_impact.json`): CS/AI 우수 국제학술대회 188개 + 인정 IF
  1–4점 (AAAI·CVPR·NeurIPS·ICML·ACL·KDD … 및 목록 이후 등장한 ICLR·COLM 보충). BK21
  4단계 PDF를 `scripts/build_venue_impact.py`로 변환해 동봉. 발표 형태가 venue 텍스트에
  드러나면 차감 규칙(Regular=IF, Short=IF−1, Spotlight=IF−2, Poster=0) 적용.
- **OpenAlex 저널 캐시** (`references/journal_impact.json`): venue별 `2yr_mean_citedness`
  (Journal Impact Factor 등가 지표)를 로그 정규화 + h-index 보정. BK21에 없는 저널(TPAMI,
  JMLR, IJCV, Nature MI …) 담당. Clarivate MJL의 JIF는 로그인·무료 대량 API 부재라
  OpenAlex 지표를 자동화 가능한 IF 대용으로 사용.

venue impact는 **기본 켜짐**이며, 두 표에 없는 논문도 코퍼스에 남는다(가산점 방식, 하드
필터 아님). AI 연구자는 `--field ai-research --goal impact`를 기본으로 쓰면 된다.

```bash
# 1) 실행의 venue들을 OpenAlex로 보강해 저널 IF를 오프라인 캐시에 채운다
python3 ~/.claude/skills/scholar-megasearch/scripts/enrich_if.py \
  ./literature_search/<topic>_<date>/raw --email you@example.com

# 2) impact 목표로 병합 — IF 높은 venue가 상위로 (venue impact 기본 켜짐)
python3 ~/.claude/skills/scholar-megasearch/scripts/merge_corpus.py \
  ./literature_search/<topic>_<date>/raw -o corpus.json --md corpus.md \
  --goal impact --topic "<주제>"

# venue 레이어를 끄려면 --no-venue-impact (나머지 5층으로 가중치 재정규화)
```
...
├── artifact/index.html     # 선택적 self-contained HTML 뷰어
└── summary.md            # 합성 리뷰
```
...
- **호스트별 Scholar Gateway는 best-effort** — 헤드리스/크론 실행에서 없을 수 있어, 어떤
  버킷의 유일한 소스로도 쓰지 않는다.
- **HTML artifact는 opt-in.** `render_artifact.py`는 기존 실행 디렉토리에서 정적
  `artifact/index.html` 리포트를 만든다. `--serve`를 명시하지 않으면 서버를 시작하지 않고,
  미리보기 서버는 기본적으로 `127.0.0.1`에 바인딩하며 sibling PDF 링크가 동작하도록 실행
  디렉토리만 서빙한다.
