...
GOALS = {
    ...
    "impact": "venue-impact first — rank high-IF venues (BK21 conf + OpenAlex journal) to the top",
}
...
def build_plan(topic, field, goal, depth):
    ...
    field = field if field in FIELDS else "cs-ml"
...
def main():
    ...
    if not args.no_input and sys.stdin.isatty():
        if not field:
            field = ask_choice("Field", sorted(FIELDS), "cs-ml")
        if not goal:
